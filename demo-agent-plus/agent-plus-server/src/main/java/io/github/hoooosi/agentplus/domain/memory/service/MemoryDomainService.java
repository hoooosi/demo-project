package io.github.hoooosi.agentplus.domain.memory.service;
...
import io.github.hoooosi.agentplus.domain.memory.model.MemoryResult;
import io.github.hoooosi.agentplus.domain.memory.model.MemoryType;
import io.github.hoooosi.agentplus.domain.memory.repository.MemoryItemRepository;
...
import static io.github.hoooosi.agentplus.domain.memory.constant.MemoryMetadataConstant.*;
...
public class MemoryDomainService {
    ...
    public List<String> saveMemories(Long userId, String sessionId, List<CandidateMemory> candidates) {
    ...
    /** 分页列出用户记忆（可按类型过滤） */
    public Page<MemoryItemEntity> pageMemories(Long userId, String type, int page, int pageSize) {
    ...
    /** 列出用户的记忆（可按类型过滤，带上限） */
    public List<MemoryItemEntity> listMemories(Long userId, String type, Integer limit) {
        var qw = Wrappers.<MemoryItemEntity>lambdaQuery().eq(MemoryItemEntity::getUserId, userId);
        if (type != null && !type.isBlank()) {
            qw.eq(MemoryItemEntity::getType, type.trim().toUpperCase());
        }
