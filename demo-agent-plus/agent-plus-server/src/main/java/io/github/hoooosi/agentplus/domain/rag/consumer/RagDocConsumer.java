...
import io.github.hoooosi.agentplus.domain.rag.strategy.context.DocumentProcessingFactory;
...
import io.github.hoooosi.agentplus.infrastructure.mq.enums.EventType;
import io.github.hoooosi.agentplus.infrastructure.mq.events.RagDocSyncOcrEvent;
...
/**
 * RAG文档OCR处理消费者 (RocketMQ)
 * @author zang
 * @date 2025-01-10
 */
@Component
@RocketMQMessageListener(
    topic = RagDocSyncOcrEvent.EXCHANGE_NAME,
    consumerGroup = "rag-doc-ocr-consumer-group",
    selectorExpression = RagDocSyncOcrEvent.ROUTE_KEY
)
public class RagDocConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String messageBody) {
        ...
        try {
            ...
            MessageEnvelope<RagDocMessage> envelope = OBJECT_MAPPER.readValue(
                    messageBody, 
            ...
            boolean completeSuccess = fileDetailDomainService.completeFileOcrProcessing(
                    docMessage.getFileId(),
                    fileEntity.getUserId()
            );
            if (!completeSuccess) {
    ...
    private void autoStartVectorization(String fileId, FileDetailEntity fileEntity) {
        try {
            ...
            // 更新向量化状态
            boolean startSuccess = fileDetailDomainService.startFileEmbeddingProcessing(
                    fileId, 
                    fileEntity.getUserId()
            );
            if (!startSuccess) {
