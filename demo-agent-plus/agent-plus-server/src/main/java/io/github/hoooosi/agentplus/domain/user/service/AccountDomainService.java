...
import io.github.hoooosi.agentplus.domain.user.repository.AccountRepository;
...
public class AccountDomainService {
    ...
    private ReentrantLock getUserLock(Long userId) {
        return userLocks.computeIfAbsent(userId.toString(), k -> new ReentrantLock());
    }

    /** 根据用户ID查找账户（私有方法）
     * @param userId 用户ID
     * @return 账户实体，如果不存在则返回null */
    private AccountEntity findAccountByUserId(Long userId) {
        if (userId == null) {
            return null;
        }
     ...
     * @param userId 用户ID
     * @return 账户实体 */
    public AccountEntity getOrCreateAccount(Long userId) {
        if (userId == null) {
