...
public class UserAuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        log.debug("JWT认证拦截器处理请求: {} {}", method, requestURI);
        ...
        try {
            ...
            if (!StringUtils.hasText(authHeader)) {
                log.warn("认证失败 - 缺少Authorization头: {} {}", method, requestURI);
                writeErrorResponse(response, "缺少认证头");
                return false;
            }

            if (!authHeader.startsWith(BEARER_PREFIX)) {
                log.warn("认证失败 - Authorization头格式错误: {} {}, header: {}", method, requestURI, authHeader);
            ...
            if (!StringUtils.hasText(token)) {
                log.warn("认证失败 - Token为空: {} {}", method, requestURI);
            ...
            if (!JwtUtils.validateToken(token)) {
                log.warn("认证失败 - Token验证失败: {} {}, token前缀: {}", method, requestURI,
            ...
            if (userId == null) {
                log.warn("认证失败 - 无法从Token中获取用户ID: {} {}", method, requestURI);
            ...
            log.debug("认证成功: {} {}, 用户ID: {}", method, requestURI, userId);

            return true;

        } catch (Exception e) {
            log.error("用户鉴权异常: {} {}, 错误: {}", method, requestURI, e.getMessage(), e);
