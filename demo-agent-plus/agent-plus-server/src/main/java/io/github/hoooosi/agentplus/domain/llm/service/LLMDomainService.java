package io.github.hoooosi.agentplus.domain.llm.service;
...
public class LLMDomainService {
    ...
    public void updateProvider(ProviderEntity provider) {
        validateProviderProtocol(provider.getProtocol());
        providerRepository.checkedUpdate(provider, Wrappers.<ProviderEntity>lambdaUpdate()
    ...
    /** 获取用户自己的服务商
     * @param userId 用户id */
    public List<ProviderAggregate> getUserProviders(Long userId) {
        List<ProviderEntity> providers = providerRepository
                .selectList(Wrappers.<ProviderEntity>lambdaQuery()
     ...
     * @param userId 用户ID
     * @return 服务商聚合根列表 */
    public List<ProviderAggregate> getAllProviders(Long userId) {
    ...
    public List<ProviderAggregate> getOfficialProviders() {
        List<ProviderEntity> providers = providerRepository.selectList(Wrappers.<ProviderEntity>lambdaQuery()
                .eq(ProviderEntity::getIsOfficial, true));
    ...
    private List<ProviderAggregate> buildProviderAggregatesWithActiveModels(List<ProviderEntity> providers) {
        ...
        // 收集服务商id
        List<Long> providerIds = providers.stream().map(ProviderEntity::getId).collect(Collectors.toList());
        // 查询激活的模型
        ...
        // 转为map，映射关系
        Map<Long, List<ModelEntity>> modelMap = activeModels.stream()
     ...
     * @param userId 用户id */
    public ProviderEntity getProvider(Long providerId, Long userId) {
    ...
    public ProviderEntity getProvider(Long providerId) {
    ...
    // 检查服务商是否存在
    public void checkProviderExists(Long providerId, Long userId) {
        ProviderEntity provider = providerRepository.selectOne(Wrappers.<ProviderEntity>lambdaQuery()
    ...
    // 获取服务商聚合根
    public ProviderAggregate getProviderAggregate(Long providerId, Long userId) {
        // 获取服务商
    ...
    // 获取模型列表
    public List<ModelEntity> getModelList(Long providerId, Long userId) {
     ...
     * @param userId 用户ID
     * @return 激活的模型列表 */
    public List<ModelEntity> getActiveModelList(Long providerId, Long userId) {
        Wrapper<ModelEntity> wrapper = Wrappers.<ModelEntity>lambdaQuery().eq(ModelEntity::getProviderId, providerId)
     ...
     * @param providerId 服务商id
     * @param userId 用户id */
    public void deleteProvider(Long providerId, Long userId, Operator operator) {
    ...
    public void updateModel(ModelEntity model) {
        modelRepository.checkedUpdate(model, Wrappers.<ModelEntity>lambdaQuery()
    ...
    /** 删除模型
     * @param modelId 模型id */
    public void deleteModel(Long modelId, Long userId, Operator operator) {
     ...
     * @param userId 用户ID
     * @return 服务商聚合根列表 */
    public List<ProviderAggregate> getProvidersByType(ProviderType providerType, Long userId) {
    ...
    private List<ProviderAggregate> buildProviderAggregatesWithAllModels(List<ProviderEntity> providers) {
        ...
        // 按服务商分组
        Map<Long, List<ModelEntity>> modelMap = allModels.stream()
                .collect(Collectors.groupingBy(ModelEntity::getProviderId));
